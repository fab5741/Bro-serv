...
function SelecteButton(menuName, number)
    ...
    assert(type(number) == "number", "[bf] SelecteButton : number must be number")
    assert(type(number) >= 1, "[bf] SelecteButton : can not be negative or zero")
...
local function menuInit()
    ...
    AddInstructionalButtons("bf_menu", {
        ["Up"] = 188,
        ["Down"] = 187,
        ["Enter"] = 201,
        ["Back"] = 177,
    ...
    Citizen.CreateThread(function()
        ...
        while true do
            ...
            if MenuIsOpen() and not IsHudComponentActive(19) and not IsHudComponentActive(16) and not IsPauseMenuActive() then
                ...
                DisplayInstructionalButtons("bf_menu")

            elseif GetCurrentInstructionalButtons() == "bf_menu" then
